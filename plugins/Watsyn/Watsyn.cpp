...
void WatsynObject::renderOutput( fpp_t _frames )
{
	...
	for( fpp_t frame = 0; frame < _frames; frame++ )
	{
		...
		if( m_amod == MOD_PM )
		{
			A1_lphase = wrap<WAVELEN>(A1_lphase + A2_L * PMOD_AMT);
			A1_rphase = wrap<WAVELEN>(A1_rphase + A2_R * PMOD_AMT);
		}
		// A1
		...
		if( m_bmod == MOD_PM )
		{
			B1_lphase = wrap<WAVELEN>(B1_lphase + B2_L * PMOD_AMT);
			B1_rphase = wrap<WAVELEN>(B1_rphase + B2_R * PMOD_AMT);
		}
		// B1
		...
		{
			m_lphase[i] += WAVELEN * m_nph->frequency() * m_parent->m_lfreq[i] / m_samplerate;
			m_lphase[i] = wrap<WAVELEN>(m_lphase[i]);
			m_rphase[i] += WAVELEN * m_nph->frequency() * m_parent->m_rfreq[i] / m_samplerate;
			m_rphase[i] = wrap<WAVELEN>(m_rphase[i]);
		}
	}
